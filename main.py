...
def with_lstm():
    ...
    architecture_with = NeuroFuzzyNetwork(
        X_train_lstm.shape[2], y_train.shape[1], Max_Neurons, Threshold_Epochs, max_neurons, threshold_epochs)
    start_time = time.time()
    history_with_lstm = architecture_with.train(
        X_train_lstm, y_train, epochs=EPOCHS)
    end_time = time.time()
...
def without_lstm():
    ...
    history_without_lstm = architecture_without.train(
        X_train_no_lstm, y_train, epochs=EPOCHS)
